...
MSSliderEvent.DESTROY				= 'destroy';//dev\slider\Slide.js 
...
(function($){
	...
	window.MSScrollParallax = function (slider, parallax, fade) {
		this.fade = fade;
		this.slider = slider;
	...
	window.MSScrollParallax.setup = function(slider, parallax, fade){
		// disable in mobile devices
		if ( window._mobile ) {
			return;
		}
		
		return new MSScrollParallax(slider, parallax, fade); 
	};
	...
	p.resetLayers = function (e) {
		if( !this.lastSlide || !this.lastSlide.$scrollParallaxCont ) {
			return;
		}

		if ( window._css2d ) {
			this.lastSlide.$scrollParallaxCont[0].style[window._jcsspfx + 'Transform'] = '';
		} else {
			this.lastSlide.$scrollParallaxCont[0].style.top = '';
		}
	};
	...
	p.moveParallax = function (e) {
		...
		// current slider doesn't have layer
		if( !layers ) {
			return;
		}
