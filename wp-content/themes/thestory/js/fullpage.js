...
"use strict";
	...
	PEXETO.Fullpage.prototype = {
		...
		init : function(){
			...
			if(this.secNum){
				...
				this.$sections.each(function(i){
					if($(this).hasClass('section-textimg')){
						self.scrollers[i]={};
					}
				});
		...
		doOnNextTrigger : function(){
			if(this.current<this.lastIndex && !this.inAnimation){
				this.moveTo(this.current+1);
			}
		},

		doOnPrevTrigger : function(){
			if(this.current !==0 && !this.inAnimation){
				this.moveTo(this.current-1);
			}
		},
		...
		doOnAfterLoad : function(link, index){
			...
			if(this.$sections.eq(secIndex).hasClass('section-slider')){
				this.animateCaption(secIndex, 0);
			}
		...
		animateElements : function(index){
			...
			if(this.animatedElements.indexOf(index)===-1){
